...
on the subsequent level. String a and b are not neccisarily on the same position
...
match, we check if there are any sub node elementns, and if none, we then use
...
codepoints passed to `push_cp_onto_stack`, we grab more and then use `subnode_link`
and `subnode_elems` to do a linear search, stopping if we see any codepointns which
