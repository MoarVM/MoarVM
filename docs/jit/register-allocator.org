...
- definitions and uses are the indexes of tiles that define or use the
  values
...
#+BEGIN_SRC c
void determine_live_ranges(MVMThreadContext *tc, MVMJitTileList *list, RegisterAllocator *alc) {
    /* implementation */
}
#+END_SRC
...
free to load spilled values; two should be sufficient for x86-64.
