...
**** TODO Factor spill-register selection out of spilling-values

**** TODO Mark ARGLIST references
...
**** TODO Special-case ARGLIST references
...
**** TODO Convert ARGLIST spec to ABI list
...
One of the complexities here is that CALL doesn't necessarily start a
live range so it's not necessarily 'seen' in the live range heap.

So we should have a queue of CALL or other register barriers, created
during the first step, and then iterate over them while running.
Matter of fact, we should probably have the same for ARGLIST, since
ARGLIST too needs special-casing.
