...
**** TODO Create a queue for special tiles
...
ALTERNATIVELY: Maintain an index `last_tile_idx'.  Prior to allocating
for a live range, iterate over all tiles. between the 'last_tile_idx'
and the start of the next live range.
