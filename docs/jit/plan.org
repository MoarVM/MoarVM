...
*** TODO Support query for live-range-in-register
...
MVMint32 transfer_queue_idx, transfer_queue_top = 0, transfers_required;
memset(rev_dep_map, 0, sizeof(rev_dep_map));
for (i = 0; i < num_args; i++) {
    LiveRange *v = alc->values + arg_values[i];
    if (v->spill_pos != 0) {
        /* spilled register, handle later */
        ...
        } else if (gpr_map[reg_num] < 0) {
            /* not in use, will not conflict */
            rev_dep_map[reg_num].dep = v->reg_num;
            rev_dep_map[reg_num].num = 0;
...
for (i = 0; i <  stack_transfer_num; i++) {
...
for (transfer_queue_idx = 0; transfer_queue_idx < transfer_queue_top; transfer_queue_idx++) {
    ...
    MVMint8 src = rev_dep_map[dst].dep;
...
if (transfer_queue_top < transfers_required) {
    /* we have cycles, break them in some way
     * we can detect a cycle by anything whose dep > 0
     *
     * we can group cycles by something resemmbling union-find,
     * e.g., when a cycle is found, mark all belonging to this; if
     * you find the one you're marking, you have the one
     */
    MVMint8 cycles[MAX_GPR];
    MVMint8 c, n;
    memset(cycles, -1, sizeof(cycles));
    for (i = 0; i < MAX_GPR; i++) {
        if (rev_dep_map[i].num > 0 && cycles[i] < 0) {
            num_cycles++;
            /* mark all belonging to this cycle */
            for (c = i; cycles[c] != i; c = rev_dep_map[c].dep) {
                cycles[c] = i;
            }
        }
    }
    for (i = 0; i < MAX_GPR; i++) {
        if (cycles[i] == i) {
            /* start of cycle, insert cyclical moves */
            insert_register_transfer(tc, alc, i, reserved_register);
            for (n = rev_dep_map[i].dep, c = i; n != i;
                 c = n, n = rev_dep_map[n].dep) {
                insert_register_transfer(tc, alc, c, n);
            }
            insert_register_transfer(tc, alc, reserved_register, i);
        }
    }
