...
* TODO Automatic unconditionalizing
...
** TODO Find 'current conditional block' for first visits

Also consists of node (IF) and child index (branch), or nothing if not
a conditional block.

** TODO Inserts roots for blocks that are used outside of their 'own' conditional
