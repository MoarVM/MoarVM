...
void __MVM_cmplog_ins_hook8(uint64_t arg1, uint64_t arg2, uint64_t caller_id, uint8_t attr) {
...
void __MVM_cmplog_rtn_hook_atkey_hook(MVMThreadContext *tc, MVMObject *hash, MVMString *str, uint64_t caller_id) {
        ...
        DISPATCH(NEXT_OP) {
           ...
           OP(bootintarray):
            ...
            OP(cmplog_i): {
                ...
                if (!tc->suppress_coverage) {
                    if (arg1 != arg2)
                        __MVM_cmplog_ins_hook8(arg1, arg2, bb_id, attrs);
                }
#endif
                cur_op += 14;
                goto NEXT;
            ...
            OP(cmplog_atkey): {
                ...
                MVMuint64 bb_id = GET_UI64(cur_op, 4);
                if (!tc->suppress_coverage) {
                    __MVM_cmplog_rtn_hook_atkey_hook(tc, hash, key, bb_id);
                }
#endif
                cur_op += 12;
                goto NEXT;
