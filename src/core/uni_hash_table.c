...
void MVM_uni_hash_demolish(MVMThreadContext *tc, MVMUniHashTable *hashtable) {
    ...
    struct MVMUniHashBucket *bucket = hashtable->buckets;
    const struct MVMUniHashBucket *const bucket_end
        = hashtable->buckets + hashtable->num_buckets;

    do {
        struct MVMUniHashHandle *head = bucket->hh_head;
        while (head) {
            struct MVMUniHashHandle *next = head->hh_next;
            MVM_fixed_size_free(tc, tc->instance->fsa, sizeof(struct MVMUniHashHandle), head);
            head = next;
        }
    } while (++bucket < bucket_end);

    MVM_fixed_size_free(tc, tc->instance->fsa,
                        hashtable->num_buckets*sizeof(struct MVMUniHashBucket),
                        hashtable->buckets);
 ...
 *
 */
void MVM_uni_hash_expand_buckets(MVMThreadContext *tc, MVMUniHashTable *tbl) {
    MVMHashBktNum he_bkt;
    MVMHashBktNum he_bkt_i;
    struct MVMUniHashHandle *he_thh, *_he_hh_nxt;
    struct MVMUniHashBucket *he_new_buckets, *_he_newbkt;
    ...
    he_new_buckets =
        MVM_fixed_size_alloc_zeroed(tc, tc->instance->fsa,
                                    new_num_bkts * sizeof(struct MVMUniHashBucket));
    ...
    for(he_bkt_i = 0; he_bkt_i < tbl->num_buckets; he_bkt_i++) {
        ...
        while (he_thh) {
            ...
            he_bkt = MVM_uni_hash_bucket(he_thh->hash, new_log2_num_buckets);
    ...
    MVM_fixed_size_free(tc, tc->instance->fsa,
                        tbl->num_buckets*sizeof(struct MVMUniHashBucket),
                        tbl->buckets);
