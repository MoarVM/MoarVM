...
void MVM_load_bytecode_buffer(MVMThreadContext *tc, MVMObject *buf) {
    ...
    if (
        ...
        || (
               ((VMArrayREPRData *)STABLE(buf)->REPR_data)->slot_type != MVM_ARRAY_U8
            && ((VMArrayREPRData *)STABLE(buf)->REPR_data)->slot_type != MVM_ARRAY_I8
        )
    )
    ...
    data_size = ((VMArray *)buf)->body.elems;
    data_start = MVM_malloc(data_size);
    memcpy(data_start, (MVMuint8 *)(((VMArray *)buf)->body.slots.i8 + ((VMArray *)buf)->body.start), data_size);
