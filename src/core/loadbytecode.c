...
void MVM_load_bytecode(MVMThreadContext *tc, MVMString *filename) {
    ...
    if (apr_thread_mutex_lock(tc->instance->mutex_loaded_compunits) != APR_SUCCESS)
        MVM_exception_throw_adhoc(tc, "Unable to lock loaded compunits");
    MVM_string_flatten(tc, filename);
    MVM_HASH_GET(tc, tc->instance->loaded_compunits, filename, loaded_name);
    if (loaded_name) {
        /* already loaded */
        if (apr_thread_mutex_unlock(tc->instance->mutex_loaded_compunits) != APR_SUCCESS)
            MVM_exception_throw_adhoc(tc, "Unable to unlock loaded compunits");
        return;
    }
