...
static void uninline(MVMThreadContext *tc, MVMFrame *f, MVMSpeshCandidate *cand,
    ...
    MVMint32 i;
    for (i = cand->num_inlines - 1; i >= 0; i--) {
        if (offset > cand->inlines[i].start && offset <= cand->inlines[i].end) {
            ...
            uf->params.legacy.callsite = cand->inlines[i].cs;
            uf->params.legacy.arg_flags = NULL;
            uf->params.named_used_size = MVM_callsite_num_nameds(tc, cand->inlines[i].cs);
            ...
            caller->return_type = cand->inlines[i].res_type;
            caller->return_value = caller->return_type == MVM_RETURN_VOID
                ? NULL
                : caller->work + cand->inlines[i].res_reg;
            caller->return_address = caller->static_info->body.bytecode +
                cand->deopts[2 * cand->inlines[i].return_deopt_idx];
        }
    }
...
void MVM_spesh_deopt_during_unwind(MVMThreadContext *tc) {
    ...
    if (deopt_idx >= 0) {
        MVMuint32 deopt_target = frame->spesh_cand->deopts[deopt_idx * 2];
        MVMuint32 deopt_offset = frame->spesh_cand->deopts[deopt_idx * 2 + 1];
