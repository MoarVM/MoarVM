...
static void optimize_guard(MVMThreadContext *tc, MVMSpeshGraph *g, MVMSpeshBB *bb,
    ...
    if (opcode == MVM_OP_sp_guard && can_drop_type_guard) {
        turn_into_set = 1;
    }
    ...
    if (turn_into_set) {
        ins->info = MVM_op_get_op(MVM_OP_set);
        fprintf(stderr, "turned a %s into a set\n\n", name);
        return;
    }
    else if ((opcode == MVM_OP_sp_guardconc || opcode == MVM_OP_sp_guardtype) && can_drop_type_guard) {
        fprintf(stderr, "could have turned a %s into the next best thing\n\n", name);
    }
}
