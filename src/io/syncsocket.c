...
void socket_connect(MVMThreadContext *tc, MVMOSHandle *h, MVMString *host, MVMint64 port) {
    MVMIOSyncSocketData *data = (MVMIOSyncSocketData *)h->body.data;
    if (!data->ss.handle) {
        char         *host_cstr = MVM_string_utf8_encode_C_string(tc, host); /* XXX Host res. */
        uv_tcp_t     *socket    = malloc(sizeof(uv_tcp_t));
        uv_connect_t *connect   = malloc(sizeof(uv_connect_t));
        struct sockaddr_in dest;
        int r;

        if ((r = uv_ip4_addr(host_cstr, port, &dest)) != 0) {
            free(socket);
            free(connect);
            free(host_cstr);
        ...
        if ((r = uv_tcp_init(tc->loop, socket)) < 0 ||
                (r = uv_tcp_connect(connect, socket, (const struct sockaddr *)&dest, on_connect)) < 0) {
            free(socket);
            free(connect);
            free(host_cstr);
            MVM_exception_throw_adhoc(tc, "Failed to connect: %s", uv_strerror(r));
