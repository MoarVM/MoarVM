...
static MVMint64 file_info_with_error(MVMThreadContext *tc, uv_stat_t* stat, MVMString *filename, MVMint32 use_lstat) {
    char * const a = MVM_process_path(tc, filename);
    uv_fs_t req;
...
void MVM_file_copy(MVMThreadContext *tc, MVMString *src, MVMString * dest) {
    char * const a = MVM_process_path(tc, src);
    char * const b = MVM_process_path(tc, dest);
    uv_fs_t req;
...
void MVM_file_link(MVMThreadContext *tc, MVMString *oldpath, MVMString *newpath) {
    uv_fs_t req;
    char * const oldpath_s = MVM_process_path(tc, oldpath);
    char * const newpath_s = MVM_process_path(tc, newpath);
...
MVMString * MVM_file_readlink(MVMThreadContext *tc, MVMString *path) {
    ...
    char * const path_s = MVM_process_path(tc, path);
