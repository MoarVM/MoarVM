...
#define HASHELEMS(vm, hash)         ((unsigned int )MVM_repr_elems(vm, arr)(vm, hash))
#define ATKEY(vm, hash, k)          (MVM_repr_at_key_boxed(vm, hash, k))
#define ATKEY_I(vm, hash, k)        (MVM_repr_get_int(tc, MVM_repr_at_key_boxed(vm, hash, k)))
#define BINDKEY(vm, hash, k, v)     (MVM_repr_bind_key_boxed(vm, hash, k, v))
#define BINDKEY_I(vm, hash, k, v)   (MVM_repr_bind_key_boxed(vm, hash, k, MVM_repr_box_int(tc, tc->instance->boot_types->BOOTInt, v)))
#define EXISTSKEY(vm, hash, k)      (MVM_repr_exists_key(vm, hash, k))
#define DELETEKEY(vm, hash, k)      (MVM_repr_delete_key(vm, hash, k))
#define EMPTY_STRING(vm)            (MVM_string_utf8_encode_C_string(tc, ""))
