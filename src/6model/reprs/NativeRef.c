...
static void compose(MVMThreadContext *tc, MVMSTable *st, MVMObject *info_hash) {
    ...
    if (IS_CONCRETE(info)) {
        ...
        MVMuint16  prim    = spec->boxed_primitive;
        if (prim != MVM_STORAGE_SPEC_BP_NONE) {
            ...
            if (IS_CONCRETE(refkind)) {
                ...
                repr_data->is_unsigned    = spec->is_unsigned;
                st->REPR_data             = repr_data;
            }
            else {
...
void MVM_nativeref_write_lex_i(MVMThreadContext *tc, MVMObject *ref_obj, MVMint64 value) {
    ...
    switch (ref->body.u.lex.type) {
        ...
        case MVM_reg_uint32:
        case MVM_reg_uint64:
            MVM_exception_throw_adhoc(tc, "Attempting to MVM_nativeref_write_lex_i (%"PRId64") to an unsigned variable", value);
            break;
        case MVM_reg_int8:
...
void MVM_nativeref_write_lex_u(MVMThreadContext *tc, MVMObject *ref_obj, MVMuint64 value) {
    ...
    switch (ref->body.u.lex.type) {
        ...
        case MVM_reg_int32:
        case MVM_reg_int64:
            MVM_exception_throw_adhoc(tc, "Attempting to MVM_nativeref_write_lex_u (%"PRIu64") to a signed variable", value);
            break;
        case MVM_reg_uint8:
