...
static void describe_refs (MVMThreadContext *tc, MVMHeapSnapshotState *ss, MVMSTable *st, void *data) {
    ...
    for (i = 0; i < body->used; i++) {
        switch (body->entries[i].kind) {
            ...
            case MVM_SPESH_LOG_DEOPT:
                MVM_profile_heap_add_collectable_rel_const_cstr_cached(tc, ss,
                    (MVMCollectable *)body->entries[i].deopt.sf, "Deopt staticframe entry", &cache_7);
                MVM_profile_heap_add_collectable_rel_const_cstr_cached(tc, ss,
                    (MVMCollectable *)body->entries[i].deopt.spesh_cand, "Deopt spesh candidate entry", &cache_8);
                break;
        }
    }
}
