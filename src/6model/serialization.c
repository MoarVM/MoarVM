...
#define init_reader reader->cur_read_buffer = &(reader->root.param_interns_data); \
...
static void resolve_param_interns(MVMThreadContext *tc, MVMSerializationReader *reader) {
    ...
    char *matches = MVM_calloc(reader->root.num_param_interns, sizeof(char));
    int matched = 1;
    while (matched) {
        matched = 0;
