...
//|.actionlist actions
static const unsigned char actions[755] = {
  ...
  137,233,255,73,139,142,233,255,72,139,145,233,255,72,139,146,233,255,72,133,
  210,15,133,244,247,255,82,81,255,76,137,252,247,72,139,116,36,8,72,199,194,
  237,73,186,237,237,65,252,255,210,255,72,131,196,16,255,72,137,194,248,1,
  255,72,137,147,233,255,72,139,137,233,72,139,147,233,72,137,145,233,255,73,
  139,140,253,36,233,72,137,139,233,255,72,139,139,233,72,141,137,233,72,131,
  185,233,0,255,15,132,244,247,72,139,137,233,255,72,131,252,249,0,255,15,133,
  244,248,255,73,139,142,233,72,139,137,233,248,2,255,72,139,139,233,72,139,
  147,233,76,141,129,233,73,131,184,233,0,15,132,244,247,77,139,128,233,248,
  1,255,102,252,247,129,233,236,15,132,244,248,72,131,252,250,0,15,132,244,
  248,255,102,252,247,130,233,236,15,133,244,248,82,65,80,76,137,252,247,255,
  72,139,179,233,73,186,237,237,65,252,255,210,65,88,90,248,2,255,73,137,144,
  233,255,72,139,139,233,72,137,139,233,255,72,139,131,233,255,72,3,131,233,
  255,72,43,131,233,255,72,15,175,131,233,255,72,153,72,252,247,187,233,255,
  72,137,131,233,255,72,252,255,131,233,255,72,252,255,139,233,255,252,242,
  ...
  72,137,131,233,255,72,59,131,233,255,15,148,208,255,15,149,208,255,15,156,
  208,255,15,158,208,255,15,159,208,255,15,157,208,255,72,15,182,192,72,137,
  131,233,255,72,139,189,233,255,72,139,181,233,255,72,139,149,233,255,72,139,
  141,233,255,76,139,133,233,255,76,139,141,233,255,76,137,252,246,255,76,137,
  ...
  142,233,255,73,139,134,233,255,72,139,56,255,72,139,48,255,72,139,16,255,
  72,139,8,255,76,139,0,255,76,139,8,255,72,139,187,233,255,72,139,179,233,
...
#line 321 "src/jit/emit_x64.dasc"
            ...
            dasm_put(Dst, 131, idx, (unsigned int)((uintptr_t)&MVM_frame_vivify_lexical), (unsigned int)(((uintptr_t)&MVM_frame_vivify_lexical)>>32));
...
#line 344 "src/jit/emit_x64.dasc"
        ...
        dasm_put(Dst, 169, Dt2(->env), Dt12([src]), Dt3([idx]));
#line 348 "src/jit/emit_x64.dasc"
    ...
    case MVM_OP_sp_getarg_i: {
        ...
        //| mov TMP1, ARGS[idx];
        //| mov WORK[reg], TMP1;
        dasm_put(Dst, 182, Dt13([idx]), Dt12([reg]));
...
#line 382 "src/jit/emit_x64.dasc"
            ...
            //| mov TMP1, MVMINSTANCE:TMP1->VMNull;
            //|2:
            dasm_put(Dst, 227, Dt1(->instance), DtA(->VMNull));
...
#line 388 "src/jit/emit_x64.dasc"
    ...
    case MVM_OP_sp_p6obind_s: {
        ...
        //|1:
        dasm_put(Dst, 238, Dt12([obj]), Dt12([val]), Dt8(->body), Dt9(->replaced), Dt9(->replaced));
#line 404 "src/jit/emit_x64.dasc"
        if (op == MVM_OP_sp_p6obind_o) {
            ...
            //| je >2; // if not, skip
            dasm_put(Dst, 266, DtC(->flags), MVM_CF_SECOND_GEN);
#line 413 "src/jit/emit_x64.dasc"
            ...
            dasm_put(Dst, 286, DtC(->flags), MVM_CF_SECOND_GEN);
#line 419 "src/jit/emit_x64.dasc"
            ...
            //|2: // done
            dasm_put(Dst, 304, Dt12([obj]), (unsigned int)((uintptr_t)&MVM_gc_write_barrier_hit), (unsigned int)(((uintptr_t)&MVM_gc_write_barrier_hit)>>32));
...
#line 429 "src/jit/emit_x64.dasc"
    ...
    case MVM_OP_set: {
         ...
         //| mov TMP1, WORK[reg2]
         //| mov WORK[reg1], TMP1
         dasm_put(Dst, 327, Dt12([reg2]), Dt12([reg1]));
#line 437 "src/jit/emit_x64.dasc"
    ...
    case MVM_OP_mod_i: {
        ...
        //| mov rax, WORK[reg_b];
        dasm_put(Dst, 336, Dt12([reg_b]));
#line 449 "src/jit/emit_x64.dasc"
        switch(ins->info->opcode) {
        case MVM_OP_add_i:
            //| add rax, WORK[reg_c];
            dasm_put(Dst, 341, Dt12([reg_c]));
#line 452 "src/jit/emit_x64.dasc"
            break;
        case MVM_OP_sub_i:
            //| sub rax, WORK[reg_c];
            dasm_put(Dst, 346, Dt12([reg_c]));
#line 455 "src/jit/emit_x64.dasc"
            break;
        case MVM_OP_mul_i:
            //| imul rax, WORK[reg_c];
            dasm_put(Dst, 351, Dt12([reg_c]));
...
#line 465 "src/jit/emit_x64.dasc"
        ...
        if (ins->info->opcode == MVM_OP_mod_i) {
            ...
            //| mov WORK[reg_a], rdx;
            dasm_put(Dst, 164, Dt12([reg_a]));
#line 470 "src/jit/emit_x64.dasc"
        } else {
            // all others in rax
            //| mov WORK[reg_a], rax;
            dasm_put(Dst, 365, Dt12([reg_a]));
...
#line 511 "src/jit/emit_x64.dasc"
    ...
    case MVM_OP_coerce_in: {
        ...
        dasm_put(Dst, 424, Dt12([src]), Dt12([dst]));
#line 519 "src/jit/emit_x64.dasc"
    ...
    case MVM_OP_coerce_ni: {
        ...
        //| mov WORK[dst], rax;
        dasm_put(Dst, 438, Dt12([src]), Dt12([dst]));
...
void MVM_jit_emit_call_c(MVMThreadContext *tc, MVMJitGraph *jg,
    ...
    for (i = 0; i < call_spec->num_args; i++) {
        ...
        case MVM_JIT_ADDR_STACK: /* unlikely to use this now, though */
            ...
            switch(i) {
                case 0:
            dasm_put(Dst, 488, -args[i].idx);
                    break;
                case 1:
            dasm_put(Dst, 493, -args[i].idx);
                    break;
                case 2:
            dasm_put(Dst, 498, -args[i].idx);
                    break;
                case 3:
            dasm_put(Dst, 503, -args[i].idx);
                    break;
                case 4:
            dasm_put(Dst, 508, -args[i].idx);
                    break;
                case 5:
            dasm_put(Dst, 513, -args[i].idx);
                    break;
                default:
                    MVM_exception_throw_adhoc(tc, "Can't JIT more than %d arguments", i);
            }
...
#line 593 "src/jit/emit_x64.dasc"
            ...
            case MVM_JIT_INTERP_FRAME:
                ...
                switch(i) {
                    case 0:
                dasm_put(Dst, 543, Dt1(->cur_frame));
                        break;
                    case 1:
                dasm_put(Dst, 548, Dt1(->cur_frame));
                        break;
                    case 2:
                dasm_put(Dst, 553, Dt1(->cur_frame));
                        break;
                    case 3:
                dasm_put(Dst, 105, Dt1(->cur_frame));
                        break;
                    case 4:
                dasm_put(Dst, 558, Dt1(->cur_frame));
                        break;
                    case 5:
                dasm_put(Dst, 563, Dt1(->cur_frame));
                        break;
                    default:
                        MVM_exception_throw_adhoc(tc, "Can't JIT more than %d arguments", i);
                }
#line 596 "src/jit/emit_x64.dasc"
            ...
            case MVM_JIT_INTERP_CU:
                ...
                //| addarg i, [rax];
                dasm_put(Dst, 568, Dt1(->interp_cu));
                switch(i) {
                    case 0:
                dasm_put(Dst, 573);
                        break;
...
#line 600 "src/jit/emit_x64.dasc"
        ...
        case MVM_JIT_ADDR_REG:
            ...
            switch(i) {
                case 0:
            dasm_put(Dst, 597, Dt12([args[i].idx]));
                    break;
                case 1:
            dasm_put(Dst, 602, Dt12([args[i].idx]));
                    break;
                case 2:
            dasm_put(Dst, 607, Dt12([args[i].idx]));
                    break;
                case 3:
            dasm_put(Dst, 612, Dt12([args[i].idx]));
                    break;
                case 4:
            dasm_put(Dst, 617, Dt12([args[i].idx]));
                    break;
                case 5:
            dasm_put(Dst, 622, Dt12([args[i].idx]));
                    break;
                default:
                    MVM_exception_throw_adhoc(tc, "Can't JIT more than %d arguments", i);
            }
#line 605 "src/jit/emit_x64.dasc"
        ...
        case MVM_JIT_ADDR_REG_F:
            ...
            switch(i) {
                case 0:
            dasm_put(Dst, 382, Dt12([args[i].idx]));
                    break;
                case 1:
            dasm_put(Dst, 627, Dt12([args[i].idx]));
                    break;
                case 2:
            dasm_put(Dst, 634, Dt12([args[i].idx]));
                    break;
                case 3:
            dasm_put(Dst, 641, Dt12([args[i].idx]));
                    break;
                case 4:
            dasm_put(Dst, 648, Dt12([args[i].idx]));
                    break;
                case 5:
            dasm_put(Dst, 655, Dt12([args[i].idx]));
                    break;
                case 6:
            dasm_put(Dst, 662, Dt12([args[i].idx]));
                    break;
                case 7:
            dasm_put(Dst, 669, Dt12([args[i].idx]));
                    break;
                default:
                    MVM_exception_throw_adhoc(tc, "Can't JIT more than %d arguments", i);
            }
#line 608 "src/jit/emit_x64.dasc"
        ...
        case MVM_JIT_ADDR_LITERAL:
            ...
            switch(i) {
                case 0:
            dasm_put(Dst, 676, args[i].idx);
                    break;
                case 1:
            dasm_put(Dst, 681, args[i].idx);
                    break;
                case 2:
            dasm_put(Dst, 686, args[i].idx);
                    break;
                case 3:
            dasm_put(Dst, 691, args[i].idx);
                    break;
                case 4:
            dasm_put(Dst, 696, args[i].idx);
                    break;
                case 5:
            dasm_put(Dst, 701, args[i].idx);
                    break;
                default:
                    MVM_exception_throw_adhoc(tc, "Can't JIT more than %d arguments", i);
            }
#line 611 "src/jit/emit_x64.dasc"
     ...
     * not sure. */
     ...
     dasm_put(Dst, 144, (unsigned int)((uintptr_t)call_spec->func_ptr), (unsigned int)(((uintptr_t)call_spec->func_ptr)>>32));
...
#line 630 "src/jit/emit_x64.dasc"
        ...
        case MVM_OP_if_i:
            ...
            dasm_put(Dst, 715, Dt12([reg]), (name));
#line 640 "src/jit/emit_x64.dasc"
        ...
        case MVM_OP_unless_i:
            ...
            //| test rax, rax;
            //| jz =>(name);
            dasm_put(Dst, 726, Dt12([reg]), (name));
