...
static const unsigned char actions[1551] = {
  ...
  15,132,244,247,73,139,150,233,72,139,146,233,72,57,209,15,132,244,247,252,
  233,245,248,1,255,72,139,139,233,73,139,148,253,36,233,72,139,146,233,255,
...
#line 866 "src/jit/emit_x64.dasc"
        ...
        case MVM_OP_ifnonnull:
            ...
            //| jmp =>(name);
            //|1:
            dasm_put(Dst, 1225, Dt11([reg]), Dt10(->instance), Dt6(->VMNull), (name));
...
void MVM_jit_emit_guard(MVMThreadContext *tc, MVMJitGraph *jg,
    ...
    dasm_put(Dst, 1257, Dt11([obj]), Dt12(->effective_spesh_slots), DtB([spesh_idx]));
...
#line 905 "src/jit/emit_x64.dasc"
        ...
        //| is_type_object TMP1;
        dasm_put(Dst, 1282, Dt7(->header.flags), MVM_CF_TYPE_OBJECT);
...
#line 917 "src/jit/emit_x64.dasc"
        ...
        //| is_type_object TMP1;
        //| jnz >1;
        dasm_put(Dst, 1298, Dt7(->header.flags), MVM_CF_TYPE_OBJECT);
...
#line 927 "src/jit/emit_x64.dasc"
    ...
    dasm_put(Dst, 1309, guard->deopt_offset, guard->deopt_target, (unsigned int)((uintptr_t)&MVM_spesh_deopt_one_direct), (unsigned int)(((uintptr_t)&MVM_spesh_deopt_one_direct)>>32));
#line 932 "src/jit/emit_x64.dasc"
    /* tell jit driver we're deopting */
    //| mov RV, MVM_JIT_CTRL_DEOPT
    //| jmp ->out;
    //|2:
    dasm_put(Dst, 1330, MVM_JIT_CTRL_DEOPT);
...
void MVM_jit_emit_invoke(MVMThreadContext *tc, MVMJitGraph *jg, MVMJitInvoke *invoke,
    ...
    dasm_put(Dst, 1341, invoke->callsite_idx, (unsigned int)((uintptr_t)&MVM_args_prepare), (unsigned int)(((uintptr_t)&MVM_args_prepare)>>32));
...
#line 950 "src/jit/emit_x64.dasc"
    for (i = 0;  i < invoke->arg_count; i++) {
        ...
        case MVM_OP_arg_o: {
            ...
            dasm_put(Dst, 1372, Dt11([src]), Dt1([dst]));
#line 961 "src/jit/emit_x64.dasc"
        ...
        case MVM_OP_argconst_i: {
            ...
            //| mov64 TMP4, val;
            //| mov REGISTER:TMP5[dst], TMP4;
            dasm_put(Dst, 1381, (unsigned int)(val), (unsigned int)((val)>>32), Dt1([dst]));
#line 969 "src/jit/emit_x64.dasc"
        ...
        case MVM_OP_argconst_s: {
            ...
            //| get_string TMP4, idx;
            //| mov REGISTER:TMP5[dst], TMP4;
            dasm_put(Dst, 1390, Dt13(->body.strings), DtA([idx]), Dt1([dst]));
#line 976 "src/jit/emit_x64.dasc"
    ...
    if (sizeof(MVMReturnType) == 4) {
        ...
        dasm_put(Dst, 1403, Dt12(->return_type), invoke->return_type);
#line 987 "src/jit/emit_x64.dasc"
    ...
    if (invoke->return_type == MVM_RETURN_VOID) {
        ...
        dasm_put(Dst, 1411, Dt12(->return_value), NULL);
#line 993 "src/jit/emit_x64.dasc"
    } else {
        //| lea TMP2, WORK[invoke->return_register];
        //| mov aword FRAME->return_value, TMP2;
        dasm_put(Dst, 1419, Dt11([invoke->return_register]), Dt12(->return_value));
