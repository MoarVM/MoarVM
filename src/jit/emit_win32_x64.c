...
//|.actionlist actions
static const unsigned char actions[687] = {
  ...
  237,255,73,139,142,233,72,139,9,72,139,137,233,72,139,137,233,72,137,139,
  ...
  72,133,210,15,133,244,247,255,82,81,255,76,137,252,241,72,139,84,36,8,73,
  ...
  255,72,139,139,233,72,141,137,233,72,131,185,233,0,255,15,132,244,247,72,
  139,137,233,255,72,131,252,249,0,255,15,133,244,248,255,73,139,142,233,72,
  139,137,233,248,2,255,72,139,139,233,72,139,147,233,76,141,129,233,73,131,
  184,233,0,15,132,244,247,77,139,128,233,248,1,255,102,252,247,129,233,236,
  15,132,244,248,72,131,252,250,0,15,132,244,248,255,102,252,247,130,233,236,
  15,133,244,248,82,65,80,76,137,252,241,255,72,139,147,233,73,186,237,237,
  72,131,252,236,32,65,252,255,210,72,131,196,32,65,88,90,248,2,255,73,137,
  ...
  137,131,233,255,72,139,141,233,255,72,139,149,233,255,76,139,133,233,255,
  76,139,141,233,255,76,137,252,242,255,77,137,252,240,255,77,137,252,241,255,
  ...
  72,199,194,237,255,73,199,192,237,255,73,199,193,237,255,252,233,244,10,255,
  252,233,245,255,72,139,131,233,72,133,192,15,133,245,255,72,139,131,233,72,
  133,192,15,132,245,255,249,255,72,139,8,72,137,139,233,255,72,139,139,233,
  72,137,8,255
};
...
static MVMuint64 try_emit_gen2_ref(MVMThreadContext *tc, MVMJitGraph *jg,
    ...
    //| mov WORK[reg], TMP1;
    dasm_put(Dst, 50, (unsigned int)((uintptr_t)obj), (unsigned int)(((uintptr_t)obj)>>32), Dt12([reg]));
...
#line 242 "src/jit/emit_x64.dasc"
    ...
    case MVM_OP_const_i64: {
        ...
        //| mov64 TMP1, val;
        //| mov WORK[reg], TMP1;
        dasm_put(Dst, 50, (unsigned int)(val), (unsigned int)((val)>>32), Dt12([reg]));
#line 249 "src/jit/emit_x64.dasc"
    ...
    case MVM_OP_const_n64: {
        ...
        //| mov64 TMP1, valbytes;
        //| mov WORK[reg], TMP1;
        dasm_put(Dst, 50, (unsigned int)(valbytes), (unsigned int)((valbytes)>>32), Dt12([reg]));
#line 257 "src/jit/emit_x64.dasc"
    ...
    case MVM_OP_const_s: {
         ...
         if (!try_emit_gen2_ref(tc, jg, (MVMObject*)s, reg, Dst)) {
             ...
             //| mov WORK[reg], TMP1;
             dasm_put(Dst, 65, Dt11(->interp_cu), Dt5(->body.strings), DtD([idx]), Dt12([reg]));
#line 270 "src/jit/emit_x64.dasc"
    ...
    case MVM_OP_null: {
        ...
        //| mov WORK[reg], TMP1;
        dasm_put(Dst, 85, Dt11(->instance), Dt9(->VMNull), Dt12([reg]));
...
#line 292 "src/jit/emit_x64.dasc"
    ...
    case MVM_OP_getlex: {
        ...
        MVMint16 i;
        //| mov TMP1, TC->cur_frame;
        dasm_put(Dst, 107, Dt11(->cur_frame));
...
#line 325 "src/jit/emit_x64.dasc"
            ...
            dasm_put(Dst, 133, idx, (unsigned int)((uintptr_t)&MVM_frame_vivify_lexical), (unsigned int)(((uintptr_t)&MVM_frame_vivify_lexical)>>32));
...
#line 348 "src/jit/emit_x64.dasc"
        ...
        dasm_put(Dst, 180, Dt1(->env), Dt12([src]), Dt2([idx]));
#line 352 "src/jit/emit_x64.dasc"
    ...
    case MVM_OP_sp_getarg_i: {
        ...
        //| mov TMP1, ARGS[idx];
        //| mov WORK[reg], TMP1;
        dasm_put(Dst, 193, Dt13([idx]), Dt12([reg]));
...
#line 386 "src/jit/emit_x64.dasc"
            ...
            //| mov TMP1, MVMINSTANCE:TMP1->VMNull;
            //|2:
            dasm_put(Dst, 238, Dt11(->instance), Dt9(->VMNull));
...
#line 392 "src/jit/emit_x64.dasc"
    ...
    case MVM_OP_sp_p6obind_s: {
        ...
        //|1:
        dasm_put(Dst, 249, Dt12([obj]), Dt12([val]), Dt7(->body), Dt8(->replaced), Dt8(->replaced));
#line 408 "src/jit/emit_x64.dasc"
        if (op == MVM_OP_sp_p6obind_o) {
            ...
            //| je >2; // if not, skip
            dasm_put(Dst, 277, DtB(->flags), MVM_CF_SECOND_GEN);
#line 417 "src/jit/emit_x64.dasc"
            ...
            dasm_put(Dst, 297, DtB(->flags), MVM_CF_SECOND_GEN);
#line 423 "src/jit/emit_x64.dasc"
            ...
            //|2: // done
            dasm_put(Dst, 315, Dt12([obj]), (unsigned int)((uintptr_t)&MVM_gc_write_barrier_hit), (unsigned int)(((uintptr_t)&MVM_gc_write_barrier_hit)>>32));
#line 431 "src/jit/emit_x64.dasc"
        }
        //| mov [TMP3+offset], TMP2; // store value into body
        dasm_put(Dst, 342, offset);
#line 433 "src/jit/emit_x64.dasc"
    ...
    case MVM_OP_set: {
         ...
         //| mov TMP1, WORK[reg2];
         //| mov WORK[reg1], TMP1;
         dasm_put(Dst, 347, Dt12([reg2]), Dt12([reg1]));
...
#line 455 "src/jit/emit_x64.dasc"
        ...
        case MVM_OP_sub_i:
            //| sub rax, WORK[reg_c];
            dasm_put(Dst, 366, Dt12([reg_c]));
#line 458 "src/jit/emit_x64.dasc"
            break;
        case MVM_OP_mul_i:
            //| imul rax, WORK[reg_c];
            dasm_put(Dst, 371, Dt12([reg_c]));
...
#line 468 "src/jit/emit_x64.dasc"
        ...
        if (ins->info->opcode == MVM_OP_mod_i) {
            ...
            //| mov WORK[reg_a], rdx;
            dasm_put(Dst, 175, Dt12([reg_a]));
#line 473 "src/jit/emit_x64.dasc"
        } else {
            // all others in rax
            //| mov WORK[reg_a], rax;
            dasm_put(Dst, 385, Dt12([reg_a]));
...
#line 522 "src/jit/emit_x64.dasc"
    ...
    case MVM_OP_coerce_ni: {
        ...
        //| mov WORK[dst], rax;
        dasm_put(Dst, 458, Dt12([src]), Dt12([dst]));
    ...
    for (i = 0; i < call_spec->num_args; i++) {
        ...
        case MVM_JIT_ADDR_STACK: /* unlikely to use this now, though */
            ...
            dasm_put(Dst, 513, -args[i].idx);
                    break;
                case 2:
            dasm_put(Dst, 518, -args[i].idx);
                    break;
                case 3:
            dasm_put(Dst, 523, -args[i].idx);
                    break;
                default:
                    MVM_exception_throw_adhoc(tc, "Can't JIT more than %d arguments", i);
            }
...
#line 596 "src/jit/emit_x64.dasc"
            ...
            case MVM_JIT_INTERP_FRAME:
                ...
                dasm_put(Dst, 107, Dt11(->cur_frame));
                        break;
                    case 1:
                dasm_put(Dst, 543, Dt11(->cur_frame));
                        break;
                    case 2:
                dasm_put(Dst, 548, Dt11(->cur_frame));
                        break;
                    case 3:
                dasm_put(Dst, 553, Dt11(->cur_frame));
                        break;
                    default:
                        MVM_exception_throw_adhoc(tc, "Can't JIT more than %d arguments", i);
                }
...
#line 602 "src/jit/emit_x64.dasc"
        ...
        case MVM_JIT_ADDR_REG:
            ...
            switch(i) {
                case 0:
            dasm_put(Dst, 577, Dt12([args[i].idx]));
                    break;
                case 1:
            dasm_put(Dst, 582, Dt12([args[i].idx]));
                    break;
                case 2:
            dasm_put(Dst, 587, Dt12([args[i].idx]));
                    break;
                case 3:
            dasm_put(Dst, 592, Dt12([args[i].idx]));
                    break;
                default:
                    MVM_exception_throw_adhoc(tc, "Can't JIT more than %d arguments", i);
            }
#line 607 "src/jit/emit_x64.dasc"
        ...
        case MVM_JIT_ADDR_REG_F:
            ...
            switch(i) {
                case 0:
            dasm_put(Dst, 402, Dt12([args[i].idx]));
                    break;
                case 1:
            dasm_put(Dst, 597, Dt12([args[i].idx]));
                    break;
                case 2:
            dasm_put(Dst, 604, Dt12([args[i].idx]));
                    break;
                case 3:
...
#line 610 "src/jit/emit_x64.dasc"
        ...
        case MVM_JIT_ADDR_LITERAL:
            ...
            switch(i) {
                case 0:
            dasm_put(Dst, 618, args[i].idx);
                    break;
                case 1:
            dasm_put(Dst, 623, args[i].idx);
                    break;
                case 2:
            dasm_put(Dst, 628, args[i].idx);
                    break;
                case 3:
            dasm_put(Dst, 633, args[i].idx);
                    break;
                default:
                    MVM_exception_throw_adhoc(tc, "Can't JIT more than %d arguments", i);
            }
#line 613 "src/jit/emit_x64.dasc"
     ...
     * not sure. */
     ...
     dasm_put(Dst, 146, (unsigned int)((uintptr_t)call_spec->func_ptr), (unsigned int)(((uintptr_t)call_spec->func_ptr)>>32));
...
#line 632 "src/jit/emit_x64.dasc"
        ...
        case MVM_OP_if_i:
            ...
            dasm_put(Dst, 647, Dt12([reg]), (name));
#line 642 "src/jit/emit_x64.dasc"
        ...
        case MVM_OP_unless_i:
            ...
            //| test rax, rax;
            //| jz =>(name);
            dasm_put(Dst, 658, Dt12([reg]), (name));
