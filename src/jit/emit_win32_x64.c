...
//|.actionlist actions
static const unsigned char actions[479] = {
  ...
  139,156,253,36,233,76,137,155,233,255,76,139,155,233,77,139,155,233,73,131,
  187,233,0,255,15,132,244,247,77,139,155,233,255,248,1,255,73,131,252,251,
  0,255,15,133,244,248,255,77,139,158,233,77,139,155,233,248,2,76,137,155,233,
  255,76,139,155,233,76,137,155,233,255,72,139,131,233,255,72,3,131,233,255,
  72,43,131,233,255,72,15,175,131,233,255,72,153,72,252,247,187,233,255,72,
  137,147,233,255,72,137,131,233,255,72,252,255,131,233,255,72,252,255,139,
  ...
  240,255,77,137,252,241,255,73,139,142,233,255,73,139,150,233,255,77,139,134,
  233,255,77,139,142,233,255,72,139,139,233,255,72,139,147,233,255,76,139,131,
  233,255,76,139,139,233,255,252,242,15,16,139,233,255,252,242,15,16,147,233,
  255,252,242,15,16,155,233,255,72,199,193,237,255,72,199,194,237,255,73,199,
  192,237,255,73,199,193,237,255,73,186,237,237,72,131,252,236,32,65,252,255,
  210,72,131,196,32,255,252,233,244,10,255,252,233,245,255,72,139,131,233,72,
  133,192,15,133,245,255,72,139,131,233,72,133,192,15,132,245,255,249,255,76,
  139,24,76,137,155,233,255,76,139,155,233,76,137,24,255
};
...
#line 252 "src/jit/emit_x64.dasc"
        ...
        //|2:
        //| mov WORK[dst], TMP;
        dasm_put(Dst, 110, Dt1(->instance), Dt9(->VMNull), Dt4([dst]));
#line 257 "src/jit/emit_x64.dasc"
    ...
    case MVM_OP_set: {
         ...
         //| mov TMP, WORK[reg2]
         //| mov WORK[reg1], TMP
         dasm_put(Dst, 125, Dt4([reg2]), Dt4([reg1]));
...
#line 285 "src/jit/emit_x64.dasc"
        ...
        case MVM_OP_add_i:
            //| add rax, WORK[reg_c];
            dasm_put(Dst, 139, Dt4([reg_c]));
#line 288 "src/jit/emit_x64.dasc"
            break;
        case MVM_OP_sub_i:
            //| sub rax, WORK[reg_c];
            dasm_put(Dst, 144, Dt4([reg_c]));
#line 291 "src/jit/emit_x64.dasc"
            break;
        case MVM_OP_mul_i:
            //| imul rax, WORK[reg_c];
            dasm_put(Dst, 149, Dt4([reg_c]));
...
#line 347 "src/jit/emit_x64.dasc"
    ...
    case MVM_OP_coerce_in: {
        ...
        dasm_put(Dst, 227, Dt4([src]), Dt4([dst]));
#line 355 "src/jit/emit_x64.dasc"
    ...
    case MVM_OP_coerce_ni: {
        ...
        //| mov WORK[dst], rax;
        dasm_put(Dst, 241, Dt4([src]), Dt4([dst]));
...
void MVM_jit_emit_call_c(MVMThreadContext *tc, MVMJitGraph *jg,
    ...
    for (i = 0; i < call_spec->num_args; i++) {
        ...
        case MVM_JIT_ADDR_STACK: /* unlikely to use this now, though */
            ...
            switch(i) {
                case 0:
            dasm_put(Dst, 291, -args[i].idx);
                    break;
                case 1:
            dasm_put(Dst, 296, -args[i].idx);
                    break;
                case 2:
            dasm_put(Dst, 301, -args[i].idx);
                    break;
                case 3:
            dasm_put(Dst, 306, -args[i].idx);
                    break;
                default:
                    MVM_exception_throw_adhoc(tc, "Can't JIT more than %d arguments", i);
            }
...
#line 426 "src/jit/emit_x64.dasc"
            ...
            case MVM_JIT_INTERP_FRAME:
                ...
                switch(i) {
                    case 0:
                dasm_put(Dst, 331, Dt1(->cur_frame));
                        break;
                    case 1:
                dasm_put(Dst, 336, Dt1(->cur_frame));
                        break;
                    case 2:
                dasm_put(Dst, 341, Dt1(->cur_frame));
                        break;
                    case 3:
                dasm_put(Dst, 346, Dt1(->cur_frame));
                        break;
                    default:
                        MVM_exception_throw_adhoc(tc, "Can't JIT more than %d arguments", i);
                }
#line 429 "src/jit/emit_x64.dasc"
        ...
        case MVM_JIT_ADDR_REG:
            ...
            switch(i) {
                case 0:
            dasm_put(Dst, 351, Dt4([args[i].idx]));
                    break;
                case 1:
            dasm_put(Dst, 356, Dt4([args[i].idx]));
                    break;
                case 2:
            dasm_put(Dst, 361, Dt4([args[i].idx]));
                    break;
                case 3:
            dasm_put(Dst, 366, Dt4([args[i].idx]));
                    break;
                default:
                    MVM_exception_throw_adhoc(tc, "Can't JIT more than %d arguments", i);
            }
#line 434 "src/jit/emit_x64.dasc"
        ...
        case MVM_JIT_ADDR_REG_F:
            ...
            switch(i) {
                case 0:
            dasm_put(Dst, 185, Dt4([args[i].idx]));
                    break;
                case 1:
...
#line 440 "src/jit/emit_x64.dasc"
     ...
     * not sure. */
     ...
     dasm_put(Dst, 412, (unsigned int)((uintptr_t)call_spec->func_ptr), (unsigned int)(((uintptr_t)call_spec->func_ptr)>>32));
