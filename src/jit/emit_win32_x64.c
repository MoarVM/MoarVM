...
static const unsigned char actions[1500] = {
  ...
  252,234,255,77,137,232,255,77,137,252,233,255,77,137,252,235,65,83,255,72,
  139,139,233,255,72,139,147,233,255,76,139,131,233,255,76,139,139,233,255,
  76,139,155,233,65,83,255,252,242,15,16,139,233,255,252,242,15,16,147,233,
  255,252,242,15,16,155,233,255,76,141,155,233,255,76,137,217,255,76,137,218,
  255,77,137,216,255,77,137,217,255,77,137,219,65,83,255,77,139,157,233,77,
  139,155,233,255,72,199,193,237,255,72,199,194,237,255,73,199,192,237,255,
  ...
  133,244,247,255,102,252,247,129,233,236,15,133,244,247,255,76,137,252,241,
  72,199,194,237,73,199,192,237,73,186,237,237,72,131,252,236,32,65,252,255,
  210,72,131,196,32,255,72,199,192,237,252,233,244,11,248,2,255,76,137,252,
  241,76,137,252,234,73,199,192,237,73,186,237,237,72,131,252,236,32,65,252,
  255,210,72,131,196,32,73,137,195,255,77,139,148,253,36,233,255,76,139,139,
  233,77,137,138,233,255,73,185,237,237,77,137,138,233,255,77,139,141,233,77,
  139,137,233,77,137,138,233,255,65,199,132,253,36,233,237,255,73,199,132,253,
  36,233,237,255,72,141,147,233,73,137,148,253,36,233,255,73,139,150,233,72,
  139,18,73,137,148,253,36,233,255,65,82,65,83,255,76,137,252,241,72,139,147,
...
#line 771 "src/jit/emit_x64.dasc"
        ...
        case MVM_JIT_REG_VAL:
            ...
            switch(i) {
                case 0:
            dasm_put(Dst, 1009, Dt11([args[i].v.reg]));
                    break;
                case 1:
            dasm_put(Dst, 1014, Dt11([args[i].v.reg]));
                    break;
                case 2:
            dasm_put(Dst, 1019, Dt11([args[i].v.reg]));
                    break;
                case 3:
            dasm_put(Dst, 1024, Dt11([args[i].v.reg]));
                    break;
                default:
            dasm_put(Dst, 1029, Dt11([args[i].v.reg]));
                     break;
            }
#line 776 "src/jit/emit_x64.dasc"
        ...
        case MVM_JIT_REG_VAL_F:
            ...
            dasm_put(Dst, 694, Dt11([args[i].v.reg]));
                    break;
                case 1:
            dasm_put(Dst, 1036, Dt11([args[i].v.reg]));
                    break;
                case 2:
            dasm_put(Dst, 1043, Dt11([args[i].v.reg]));
                    break;
                case 3:
            dasm_put(Dst, 1050, Dt11([args[i].v.reg]));
                    break;
                default:
            dasm_put(Dst, 1029, Dt11([args[i].v.reg]));
                    break;
...
#line 779 "src/jit/emit_x64.dasc"
        ...
        case MVM_JIT_REG_ADDR:
            ...
            //| addarg i, TMP6;
            dasm_put(Dst, 1057, Dt11([args[i].v.reg]));
            switch(i) {
                case 0:
            dasm_put(Dst, 1062);
                    break;
...
#line 783 "src/jit/emit_x64.dasc"
        ...
        case MVM_JIT_STR_IDX:    
            ...
            //| addarg i, TMP6;
            dasm_put(Dst, 1084, Dt13(->body.strings), DtA([args[i].v.lit_i64]));
            switch(i) {
                case 0:
            dasm_put(Dst, 1062);
                    break;
...
#line 790 "src/jit/emit_x64.dasc"
        ...
        case MVM_JIT_LITERAL_PTR:
            ...
            //| addarg i, TMP6;
            dasm_put(Dst, 1120, (unsigned int)(args[i].v.lit_i64), (unsigned int)((args[i].v.lit_i64)>>32));
            switch(i) {
                case 0:
            dasm_put(Dst, 1062);
                    break;
...
#line 813 "src/jit/emit_x64.dasc"
    ...
    case MVM_JIT_RV_DEREF:
        ...
        dasm_put(Dst, 1125, Dt11([call_spec->rv_idx]));
#line 817 "src/jit/emit_x64.dasc"
    ...
    case MVM_JIT_RV_ADDR:
        ...
        //| mov [RV], TMP1;
        dasm_put(Dst, 1133, Dt11([call_spec->rv_idx]));
...
#line 836 "src/jit/emit_x64.dasc"
        ...
        case MVM_OP_if_i:
            ...
            dasm_put(Dst, 1150, Dt11([reg]), (name));
#line 846 "src/jit/emit_x64.dasc"
        ...
        case MVM_OP_unless_i:
            ...
            //| test rax, rax;
            //| jz =>(name);
            dasm_put(Dst, 1161, Dt11([reg]), (name));
...
void MVM_jit_emit_guard(MVMThreadContext *tc, MVMJitGraph *jg,
    ...
    dasm_put(Dst, 1172, Dt11([obj]), Dt12(->effective_spesh_slots), DtB([spesh_idx]));
...
#line 880 "src/jit/emit_x64.dasc"
        ...
        //| is_type_object TMP1;
        dasm_put(Dst, 1197, Dt7(->header.flags), MVM_CF_TYPE_OBJECT);
...
#line 892 "src/jit/emit_x64.dasc"
        ...
        //| is_type_object TMP1;
        //| jnz >1;
        dasm_put(Dst, 1213, Dt7(->header.flags), MVM_CF_TYPE_OBJECT);
...
#line 907 "src/jit/emit_x64.dasc"
    ...
    //| jmp ->out;
    //|2:
    dasm_put(Dst, 1254, MVM_JIT_CTRL_DEOPT);
...
void MVM_jit_emit_invoke(MVMThreadContext *tc, MVMJitGraph *jg, MVMJitInvoke *invoke,
    ...
    dasm_put(Dst, 1265, invoke->callsite_idx, (unsigned int)((uintptr_t)&MVM_args_prepare), (unsigned int)(((uintptr_t)&MVM_args_prepare)>>32));
...
#line 925 "src/jit/emit_x64.dasc"
    for (i = 0;  i < invoke->arg_count; i++) {
        ...
        case MVM_OP_arg_o: {
            ...
            dasm_put(Dst, 1305, Dt11([src]), Dt1([dst]));
#line 936 "src/jit/emit_x64.dasc"
        ...
        case MVM_OP_argconst_i: {
            ...
            //| mov64 TMP4, val;
            //| mov REGISTER:TMP5[dst], TMP4;
            dasm_put(Dst, 1314, (unsigned int)(val), (unsigned int)((val)>>32), Dt1([dst]));
#line 944 "src/jit/emit_x64.dasc"
        ...
        case MVM_OP_argconst_s: {
            ...
            //| get_string TMP4, idx;
            //| mov REGISTER:TMP5[dst], TMP4;
            dasm_put(Dst, 1323, Dt13(->body.strings), DtA([idx]), Dt1([dst]));
#line 951 "src/jit/emit_x64.dasc"
    ...
    if (sizeof(MVMReturnType) == 4) {
        ...
        dasm_put(Dst, 1336, Dt12(->return_type), invoke->return_type);
...
#line 975 "src/jit/emit_x64.dasc"
    ...
    dasm_put(Dst, 1336, Dt12(->jit_entry_label), invoke->reentry_label);
...
#line 983 "src/jit/emit_x64.dasc"
        ...
        dasm_put(Dst, 1382, Dt11([invoke->code_register]), (unsigned int)((uintptr_t)&MVM_frame_find_invokee_multi_ok), (unsigned int)(((uintptr_t)&MVM_frame_find_invokee_multi_ok)>>32));
...
#line 997 "src/jit/emit_x64.dasc"
        ...
        //| callr FUNCTION;
        dasm_put(Dst, 1433, Dt7(->st), Dt9(->invoke));
#line 1001 "src/jit/emit_x64.dasc"
        ...
        dasm_put(Dst, 1455, Dt11([invoke->code_register]), invoke->spesh_cand, (unsigned int)((uintptr_t)&MVM_frame_invoke_code), (unsigned int)(((uintptr_t)&MVM_frame_invoke_code)>>32));
